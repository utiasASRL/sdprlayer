...
class TestLocalize(unittest.TestCase):
    ...
    def test_grad_and_soln_scalar(t):
        ...
        # Different methods for computing gradients and their tolerances [error, jacobian]
        estimator_tols = {
            "sdpr-sdp": [1e-5, 2e-4],
            "sdpr-cift": [1e-5, 5e-5],
            "sdpr-is": [1e-5, 5e-5],
        ...
        jac_diffs, errors = process_grad_data(
            df=df, experiment="stereo-sclwt", return_summary=False
        )
        # Loop over estimators
        for estimator in estimator_tols.keys():

            np.testing.assert_allclose(
                errors[estimator],
                np.zeros(errors[estimator].shape),
                atol=estimator_tols[estimator][0],
                err_msg=f"Jacobian error too large for {estimator}",
            )
            # Test that the normalized infinity norm of the jacobian is below an acceptable level.
            np.testing.assert_allclose(
                jac_diffs[estimator],
                np.zeros(jac_diffs[estimator].shape),
                atol=estimator_tols[estimator][1],
                err_msg=f"Jacobian error too large for {estimator}",
            )
