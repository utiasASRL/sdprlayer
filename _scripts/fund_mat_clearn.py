...
class FundMatLifter(StateLifter):
    ...
    def get_x(self, theta=None, parameters=None, var_subset=None) -> np.ndarray:
        ...
        feas_pt = np.vstack([vecF, epipole, np.array([[1]])])
        return feas_pt
...
def gen_learned_constraints():
    """Generate the learned set of constraints"""
    # Define lifter
    lifter = FundMatLifter()
    # Define list of known constraints
    A_known = []
    # A_known += FundMatSDPBlock.get_nullspace_constraint()
    # A_known += FundMatSDPBlock.get_epi_norm_constraints()
    # A_known += FundMatSDPBlock.get_fund_norm_constraint()
