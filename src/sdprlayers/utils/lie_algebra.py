...
def so3_log(R):
    ...
    if large_angles_num > 0:
        ax_sin = axes[large_angles_indices] / sin_angles[
            large_angles_indices
        ].expand_as(axes[large_angles_indices])
        logs[large_angles_indices, :] = (
            0.5 * angles[large_angles_indices].expand_as(ax_sin) * ax_sin
        )
...
def so3_inv_left_jacobian(phi):
    ...
    if large_angles_num > 0:
        ...
        cot_half_angles = 1.0 / torch.tan(half_angles)
        ...
        invJ[large_angles_indices] = (
            h_a_cot_a * I_full
            + (1 - h_a_cot_a) * outer_prod_axes
            - (h_a * so3_wedge(axes))
        )
...
def so3_to_rpy(rot):
    ...
    pitch = torch.atan2(
        -rot[:, 2, 0], torch.sqrt(rot[:, 0, 0] ** 2 + rot[:, 1, 0] ** 2)
    )

    sec_pitch = 1.0 / torch.cos(pitch)
    ...
    if pospi2_angles_num > 0:
        yaw[pospi2_indices] = 0.0
        roll[pospi2_indices] = torch.atan2(
            rot[pospi2_indices, 0, 1], rot[pospi2_indices, 1, 1]
        )
...
def se3_Q(rho, phi):
    ...
    m1 = 0.5
    m2 = (ph - sph) / ph3
    m3 = (ph2 + 2.0 * cph - 2.0) / (2.0 * ph4)
    m4 = (2.0 * ph - 3.0 * sph + ph * cph) / (2.0 * ph5)

    t1 = m1 * rx
    t2 = m2 * (px.bmm(rx) + rx.bmm(px) + px.bmm(rx).bmm(px))
    t3 = m3 * (px.bmm(px).bmm(rx) + rx.bmm(px).bmm(px) - 3.0 * px.bmm(rx).bmm(px))
